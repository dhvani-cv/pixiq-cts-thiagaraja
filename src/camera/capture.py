...
class CaptureSequence:
    ...
    def capture_part(self) -> CapturedImages:
        ...
        Flush strategy — ALL flushing happens once at cycle start, before
        any sensor has fired for this cone. The sensors on this rig are
        physically close together: VL (and even Tail) can legitimately
        fire while UV capture is still blocking. Flushing a downstream
        camera between captures would therefore discard the REAL frame
        for this cone and force a timeout. So:

            1. Flush all 3 buffers up front — anything present now is
               guaranteed stale (previous cycle bounce, idle-time noise).
            2. Capture UV → VL → Tail with NO flushes in between. A frame
               already waiting in a downstream buffer is this cone's
               legitimate frame and is returned immediately.
            3. After the final (Tail) capture, flush Tail once more to clear
               any sensor-bounce double-trigger so it doesn't sit stale
               into the next cycle. (OutputQueueSize=1 already auto-drops
               older frames, so this is cheap insurance for the case
               where that node write failed at connect time.)

        With cycle_start gating, PLC only releases one cone at a time, so
        any frame that arrives after the up-front flush belongs to this
        cone.
        ...
        # Flush all enabled camera buffers at cycle start — before any sensor
        # has fired for this cone. This is the ONLY safe flush point: once UV
        # capture begins, a frame appearing in VL/Tail buffers is real.
        for label, cam in active:
            try:
                cam.flush_buffers()
            except Exception as e:
                logger.warning("%s flush_buffers failed: %s", label, e)
        ...
        if self.cam_tail is None:
            ...
            try:
                ...
                # don't sit stale in the buffer for the next cycle.
