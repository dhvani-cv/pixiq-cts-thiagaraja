...
class TubePatternResult:
    """Result of tube pattern verification against the expected material's
    template only (never classification against other materials).

    When a PatternVerifier model is configured, the pass/fail decision comes
    from verifier_probability (a learned logistic-regression fusion of
    color+FFT+ResNet distances) vs. the model's threshold. Without a model,
    it falls back to the hand-weighted combined_distance vs. threshold.
    The raw color/fft/resnet distance fields are kept for logging/debugging
    only — the UI should show verifier_probability, not these.
    """
    color_nearest: str           # Set to the expected material_id in verification mode
    color_distance: float        # Bhatt distance to the expected template
    color_match: bool            # Final pass/fail decision (verifier or combined_distance)
    resnet_nearest: str          # Unused in verification mode (kept for compatibility)
    resnet_distance: float       # Cosine distance to the expected template's ResNet feature
    resnet_match: bool           # Unused in verification mode (kept for compatibility)
    expected_class: str = ""
    reference_loaded: bool = True
    combined_nearest: str = ""   # Set to the expected material_id in verification mode
    combined_distance: float = 0.0  # Hand-weighted bhatt+fft distance (fallback formula only)
    fft_distance: float = 0.0   # FFT cosine distance to the expected template
    verifier_probability: float = 0.0  # PatternVerifier match probability, 0.0 if no model configured
